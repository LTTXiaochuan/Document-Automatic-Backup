...
#include <algorithm>
#include <codecvt>
#include <processthreadsapi.h> // 确保包含这个头文件
...
void BackupManager::stopWatching() {
    ...
    // 通知轮询线程停止等待（如果你有条件变量cv，记得notify_all）
    cv.notify_all();

    if (hDir != INVALID_HANDLE_VALUE && watchThread) {
        // 获取线程ID
        auto nativeHandle = watchThread->native_handle(); // MSVC下是 uintptr_t 线程句柄
        // 线程ID是 DWORD，可以用 GetThreadId
...
void BackupManager::watchLoop() {
    if (pollingMode) {
        ...
        while (watching) {
            ...
            // 替换 sleep_for，使用条件变量等待，便于立即响应退出
    ...
    // 非轮询模式，使用 ReadDirectoryChangesW 监听目录变化
