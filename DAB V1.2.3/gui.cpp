...
void AddTrayIcon(HWND hWnd) {
    ...
    wcscpy_s(nid.szTip, L"Document Automatic Backup V1.2.3");
...
void LoadConfig(std::wstring& sourcePath, std::wstring& targetsMultiLine) {
    ...
    // 处理 POLLING 配置项
    size_t pollingPos = targetsMultiLine.find(L"POLLING=");
    if (pollingPos != std::wstring::npos) {
        std::wstring pollingLine = targetsMultiLine.substr(pollingPos);
        size_t lineEnd = pollingLine.find_first_of(L"\r\n");
        if (lineEnd != std::wstring::npos) {
            pollingLine = pollingLine.substr(0, lineEnd);
        }

        if (pollingLine == L"POLLING=1") {
            backupMgr.pollingMode = true;
        } else {
            backupMgr.pollingMode = false;
        }

        // 移除 POLLING 行本身
        targetsMultiLine.erase(pollingPos, pollingLine.size());
    } else {
        backupMgr.pollingMode = false;
    }

    // 查找 POLLING_INTERVAL 行
    size_t intervalPos = targetsMultiLine.find(L"POLLING_INTERVAL=");
    if (intervalPos != std::wstring::npos) {
        size_t lineEnd = targetsMultiLine.find_first_of(L"\r\n", intervalPos);
        std::wstring intervalLine = targetsMultiLine.substr(intervalPos, lineEnd - intervalPos);

        int interval = std::stoi(intervalLine.substr(17)); // "POLLING_INTERVAL=".length() == 17
        backupMgr.setPollingInterval(interval);

        // 移除该行
        if (lineEnd != std::wstring::npos)
            targetsMultiLine.erase(intervalPos, lineEnd - intervalPos + 1);
        else
            targetsMultiLine.erase(intervalPos);
    } else {
        backupMgr.setPollingInterval(2000); // 默认值
    }
    ...
    switch (msg) {
        ...
        case WM_CREATE: {
            ...
            HWND hCopyright = CreateWindowW(L"static", L"Created by Xiaochuan © 2025",
                ...
                250, 345, 200, 20, hwnd, NULL, g_hInstance, NULL);
            ...
            hLogBox = CreateWindowW(L"edit", L"", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_MULTILINE | WS_VSCROLL | ES_AUTOVSCROLL,
                10, 215, 440, 120, hwnd, (HMENU)ID_LOG_BOX, g_hInstance, NULL);
            ApplyUIFont(hLogBox);
                    ...
                    if (!PathStripToRootW(rootPath)) {
                        ...
                        if (GetVolumeInformationW(rootPath, NULL, 0, NULL, NULL, NULL, fsName, MAX_PATH)) {
                            ...
                            if (attr != INVALID_FILE_ATTRIBUTES &&
                                ...
                                std::wstring msg = L"⚠️ 当前路径所在磁盘为 " + std::wstring(fsName) +
...
int RunBackupApp(HINSTANCE hInstance, int nCmdShow) {
    ...
    hMainWnd = CreateWindowExW(
        ...
        CW_USEDEFAULT, CW_USEDEFAULT, 470, 405,
        NULL,
        NULL,
        hInstance,
        NULL
